classdef RatPol
   ...
   %   r(x,y) = (a x^2 + b xy + c y^2 + d x + e y + f) / (g x + h y + k).
   % This is the type of the CONVEX ENVELOPE (biconjugate f**) of a PLQ function: by
   %   [COAP] Karmarkar & Lucet, Comput. Optim. Appl. 94 (2026) 747-780 (Step 4) and
   %   [JOGO] Karmarkar & Lucet, J. Glob. Optim. 94 (2026) 3-34 (Prop. 1),
   ...
   % conj/partialConj/biconj therefore raise a clear error here. Continuity extension of a
   % rational piece where the denominator vanishes (cf. cPLQ/vertexNan.m) is a TODO: eval
   % returns NaN at such points for now.
   %
   % based on
   ...
   properties
        ...
        f (:,10){mustBeNumeric} % nf x 10 matrix storing the coefficients of the NUMERATOR (cubic basis)
            %f(k,:)=c means at the point (x,y) in the face k  the numerator value
            ...
            % The value of the function on face k is numerator / denominator = (f(k,:) basis) / den(k,:).
   ...
   % Class methods
   methods
       function obj = RatPol(varargin) % constructor
            % Signatures:
            ...
            switch nargin
                ...
                case 2, fc = varargin{1}; denIn = varargin{2};
                case 4, fc = varargin{3};
                case 5, fc = varargin{3}; denIn = varargin{5};
                otherwise
                    error('RatPol:nargin','RatPol accepts 1, 2, 4, or 5 arguments but got %i.', nargin);
            end
            [n1,n2] = size(fc);
            obj.f = [zeros(n1,10-n2), fc];%pad with zeros for noncubic numerators
            if nargin <= 2 %full-domain function entered as f (and optional den)
                ...
                if ~all(i), error(sprintf("Invalid matrix size in RatPol constructor. Validation vector: [V, E, f, F]=[%i, %i, %i, %i]",i));end %#ok<SPERR>
                obj.nv=nv; obj.ne=ne; obj.nf=nf;
                obj.P = obj.createP();
                %fix nf for degenerate cases
                if ismember([obj.nv,obj.ne],[1,0;2,1],'rows')
                    %needle (1,0) / segment-ray (2,1)
                    obj.nf=0;
                end
                if max(obj.F,[],'all')==0 %domain has dimension < 2
                    obj.nf=0;
                end
            ...
            % denominators: default to [0 0 1] (=1) per numerator row; validate size nf x 3
            if isempty(denIn)
                obj.den = repmat([0 0 1], n1, 1);
            else
                if size(denIn,2)~=3 || size(denIn,1)~=n1
                    error('RatPol:denSize','den must be %i x 3 (one linear denominator per face) but is %i x %i.', ...
                        n1, size(denIn,1), size(denIn,2));
                end
                obj.den = denIn;
            end
       ...
       function [fVal,region] = eval(obj,x)
        ...
        % [input]  obj of type RatPol to access V, E, f, P
        ...
        %               polytope where the value is infinity.
            ...
            if obj.nv==0%function defined everywhere
                region = ones(size(x,1),1);
                fVal = RatPol.evalRational(obj.f, obj.den(1,:), x);
                return;
            elseif max(obj.F,[],'all')==0 %edge chain
                fVal=Inf * ones(size(x,1),1);
                for i=1:obj.ne%loop on each edge
                    b = RatPol.belongToEdge(obj.V(obj.E(i,1),:),obj.V(obj.E(i,2),:),x);%true if x belongs to edge i
                    if any(b)
                        R = RatPol.evalRational(obj.f(i,:), obj.den(i,:), x(b,:));
                        ib=find(b);
                        for j=1:length(ib)
                           if isinf(fVal(ib(j)))
                               fVal(ib(j)) = R(j);
            ...
            E3 = edges(m/2+1:m,1);
            E4 = edges(m/2+1:m,2);
            coefficients = RatPol.lineEquation([E1 E2],[E3 E4]);%[-(E4-E2), E3-E1, E1.*E4-E3.*E2];
       ...
       function dom = createDom(obj)
           % return the domain of the RatPol function; in addition store it in obj.Dom
           ...
           if obj.nf==0%dim(domain)=1: segment, ray, boundary of slice
               ...
               if (obj.nv==2) && (obj.ne==1) %segment or ray
                   ...
                   if RatPol.isCollinear(obj.V,sqrt(eps))
                       dom.isConvex=true;
                   end
       ...
       %output: isConvex: true if the face is convex and the function is convex on that face
            ...
            if sum(abs(C),'all') < sqrt(eps)  %quadratic function; treat as special case for clarity AND performance
                ...
                isConvex = RatPol.isPositiveSemidefinite(Q);
                return;
                ...
                H = RatPol.evalHessian(obj.f(k,:),Vect);%returns hypermatrix of size 2x2xsize(Vect,1)
                isConvex = RatPol.isPositiveSemidefinite(H);
            end
       ...
       %        isContinuous: true if the function is continuous accross the edge
            ...
            if k==0%case function is only defined on the edge
                ...
                isConvex = RatPol.isCubicConvexOnEdge(obj.f(j,:),obj.V(obj.E(j,1),:),obj.V(obj.E(j,2),:),obj.E(j,3));
                return;
            end
            ...
            c = RatPol.lineEquation(V(1,:),V(2,:));a=c(1,1:2)'; beta=c(1,3);
            
            [~,g1] = RatPol.evalHessian(obj.f(obj.F(j,1),:),V);
            [~,g2] = RatPol.evalHessian(obj.f(obj.F(j,2),:),V);
            delta1 = (g1(:,1)-g2(:,1)) ./ a;
...
%                 %Theorem 3.8, p. 28.                
%                 [L1,Q1] = RatPol.matrixForm(obj.f(obj.F(j,1),:));
%                 [L2,Q2] = RatPol.matrixForm(obj.f(obj.F(j,2),:));
% 
...
%             expand(g)
       ...
       %output: b: boolean; true if the PC function is continuous accross edge j
            ...
            fVal1 = RatPol.evalPoly(c1,X);fVal2=RatPol.evalPoly(c2,X);
       ...
       %   RatPol is quadratic on a polyhedral subdivision, so operators require degree<=2.
       ...
       function g = conj(obj, engine) %#ok<INUSD>
       % objective: Legendre-Fenchel conjugate f*(s) = sup_x <s,x> - f(x).
       %   For a RatPol (rational on polyhedral = a convex envelope f**), the conjugate is f*
       %   (a QuaPar). This requires the general conjugate pipeline and is not implemented yet.
            error('RatPol:conj:notImplemented', ...
                ['Conjugate of a RatPol (rational quad/linear on polyhedral) is a QuaPar and ' ...
                 'requires the general conjugate pipeline (not implemented yet). See DESIGN.md II.5.1.']);
       end
       function g = partialConj(obj, idx, engine) %#ok<INUSD>
       % objective: partial conjugate -- not implemented for RatPol yet (see conj).
            error('RatPol:partialConj:notImplemented', ...
                'Partial conjugate of a RatPol is not implemented yet. See DESIGN.md II.5.1.');
       end
       function h = biconj(obj, engine) %#ok<INUSD>
       % objective: biconjugate f** = conj(conj(f)) -- not implemented for RatPol yet (see conj).
   ...
   methods(Static)
        ...
        %       isSegment: true: segment between v1 and v2, false: ray from v1 passing thru v2 till infinity
            ...
            coeff = RatPol.lineEquation(v1,v2);
        ...
        function z = evalRational(c, den, X)
        % objective: evaluate the rational function (numerator c) / (linear denominator den) at X
        % [input]  c   : 1xn numerator coefficients (n<=10), in the same weighted basis as evalPoly
        %          den : 1x3 linear denominator [g h k0] giving g*x + h*y + k0
        %          X   : kx2 matrix of points
        % [output] z   : kx1 values; entries where the denominator is ~0 are set to NaN
        %                (continuity extension at such points -- cf. cPLQ/vertexNan.m -- is a TODO).
            num = RatPol.evalPoly(c, X);
            d   = den(1)*X(:,1) + den(2)*X(:,2) + den(3);
            z = num ./ d;
            z(abs(d) < sqrt(eps)) = NaN;
        end
        ...
        % output b: n x 1 boolean vector with b(i) = true if X(i,:) belongs to segment [V1,V2]
            ...
            for i=1:size(X,1)%isCollinear is vectorized on X, but not on V1 and V2 so need to loop
                b(i) = RatPol.isCollinear([V1;V2;X(i,:)],tol);
            end
        ...
        function p = energy()%half the norm squared
            p = RatPol([1 0 1 0 0 0]);%x^2/2 + y^2/2
        end
        function p = fullRational()
        % Full-domain rational r(x,y) = (x^2 + y^2) / (x + y + 3).
            p = RatPol([2 0 2 0 0 0], [1 1 3]);   % numerator x^2+y^2, denominator x+y+3
        end
        function p = squareRational()
        % r(x,y) = (x^2 + y^2)/(x + y + 3) on the unit square [0,1]^2, +inf outside.
            V = [0 0; 1 0; 1 1; 0 1];
            E = [1 2 1; 2 3 1; 3 4 1; 4 1 1];
            F = [1 0; 1 0; 1 0; 1 0];
            f = [2 0 2 0 0 0];
            den = [1 1 3];
            p = RatPol(V,E,f,F,den);
        end
        function p = cubic1()
        ...
        function P = examples()
           %P=RatPol.examples; for i=1:length(P),plot(P{i});title(i+": "+P{i}.nf+" faces");pause;end
           % 1 One norm
            P{1}=RatPol.oneNorm();
            P{2}=RatPol.oneNormConjugate();
            P{3}=RatPol.energy();
            P{4}=RatPol.cubic1();

...
| **Unbounded** multi-face domain, a face with a **curved convex** envelope | **OK** — STALE ROW CORRECTED 2026-08-25. This is what `conjConvexOverPiece` was written for: the KKT active-set decomposition covers the bounded triangle and the unbounded wedge/half-strip with the same code, because a ray contributes its direction exactly as a bounded edge does. `plq_1p.conjugate`'s own comment says so, and `unboundedFaceTest/convexFaceOverAWedgeGoesThroughTheWholePipeline` is green on it and records that it *used to* raise the identifier below | `functionNDomain` cells | — |
| **Unbounded** face whose envelope is neither affine nor convex (i.e. RATIONAL, the nCE==1 / A.4 form) | **GAP**, reachability UNKNOWN — this is what the guard actually tests (`envUnbounded && ~envIsAffine`, after the convex branch has returned). No `QuaPol` input has been found that reaches it: for an indefinite quadratic over an unbounded face the recession directions force the envelope to be affine (`x*y` on the first quadrant and on the half-strip `0<=x<=1, y>=0` both give conv f = 0) or `-inf` (`x*y` on `y>=0`), and both of those are handled above. Classify as N/R only when someone proves it, or when the general rule under Step 1 does | — | `plq_1p:conjugateFunction:unboundedNonAffine` |
| Step 3 handed a fan triangle whose OWN Step 2 fell back to symbolic | **OK** (fallback) — ROW CORRECTED 2026-08-25, in both its description and its class. It was written as "Step 3 with a non-triangular envelope piece"; the guard is in `conjPolygonalDomain`'s fan loop and fires when `conjSingleTriangle` hands back a `QuaParCPLQ`, i.e. that triangle's Step 2 went symbolic and Step 3's NUMERIC max cannot mix the two representations. Refusing to mix them is correct, and it is not a gap: `conjCPLQ` lists this identifier in its fallback catch, so under `auto` the whole domain routes to Case C and is answered. It is a symbolic FALLBACK, and it shrinks as the Step 2 fallbacks do | `QuaParCPLQ` via Case C | numeric route only: `PLQ:conjCPLQ:notImplemented` |

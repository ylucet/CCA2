...
| `QuaPar.eval` wrong at ~0.8% of CURVED result vertices (9/1105) | `maxQuaPar.m` header, `DESIGN.md`, section 7 | **still not reproducible.** `sweepQuaParEvalAtVertices` deliberately does not cover it: generating valid random parabolic subdivisions (orientation invariant, `b^2-4ac = 0`, arcs that are genuine arcs) is separate work, and manufacturing them by running `maxQuaPar` is the fragile route that failed in 2026-08-01's attempt |

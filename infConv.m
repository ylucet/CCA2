...
% [output] h      : QuaPol | QuaPar, the inf-convolution f box g
%
...
%
% METHOD: conj(f,engine) and conj(g,engine) may each come back as either QuaPol (conjCPLQ's
%   full-domain-quadratic shortcut) or QuaPar (its general single-piece case) -- see conjCPLQ.m's
%   own header. QuaPol.add and QuaPar.add only accept same-class operands, so both conjugates
%   are first promoted to QuaPar (toQuaPar.m): QuaPol and QuaPar share the same V/E/f/F layout

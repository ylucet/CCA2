classdef RatParTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function theLatticeMatchesTheMathematics(testCase)
            ...
            %   QuaPoly < RatPol & QuaPar   -- a genuine diamond, because the four named types are
            ...
            types = {'Rat','Qua','Par','Pol','RatPar','RatPol','QuaPar','QuaPoly'};
            expected = [ ...
                1 1 1 1 1 1 1 1     % QuaPoly    -- everything: bottom of the lattice
        ...
        function traitsAnswerEachAxisIndependently(testCase)
            ...
            % enumerating combinations. `isa(f,'Qua')` replaces `isa(f,'QuaPoly')||isa(f,'QuaPar')`,
            % and keeps working when a type is added later.
            V = [0 0; 1 0; 0 1]; E = [1 2 1; 2 3 1; 3 1 1]; F = [1 0; 1 0; 1 0];
            quaPoly = QuaPoly(V, E, [0 1 0 0 0 0], F);
            ratPol  = RatPol(V, E, [0 1 0 0 0 0], F);
            quaPar  = QuaPar(V, E, zeros(3,6), [0 1 0 0 0 0], F);

            % function axis
            testCase.verifyTrue(isa(quaPoly,'Qua'));   testCase.verifyTrue(isa(quaPar,'Qua'));
            testCase.verifyFalse(isa(ratPol,'Qua'));
            % subdivision axis
            testCase.verifyTrue(isa(quaPoly,'Pol'));   testCase.verifyTrue(isa(ratPol,'Pol'));
        ...
        function pinnedValuesAreEnforcedByTraitKeyedValidators(testCase)
            ...
            % QuaPoly < RatPol & QuaPar impossible. So the pinning cannot be expressed by
            ...
            testCase.verifyError(@() setfield(quaPoly, 'den', [1 2 3]), ...
            ...
            testCase.verifyError(@() setfield(quaPoly, 'Ec', ones(3,6)), ...
        ...
        function everyObjectIsFullyFormedWithRespectToTheLattice(testCase)
            % A QuaPoly really IS a RatPol with unit denominators and a QuaPar with zero conics, so
            ...
            % QuaPoly genuinely answers true to that test.
            V = [0 0; 1 0; 0 1]; E = [1 2 1; 2 3 1; 3 1 1]; F = [1 0; 1 0; 1 0];
            quaPoly = QuaPoly(V, E, [0 1 0 0 0 0], F);
            testCase.verifyEqual(quaPoly.den, [0 0 1], 'AbsTol', 0);
            testCase.verifyTrue(all(quaPoly.Ec(:) == 0));
            testCase.verifySize(quaPoly.Ec, [3 6]);

            % and the guard that motivated it does not trip
            testCase.verifyTrue(isa(quaPoly, 'RatPol'));
            testCase.verifyFalse(any(abs(quaPoly.den(1,1:2)) > sqrt(eps)));

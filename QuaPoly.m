classdef QuaPoly
   ...
   % storable and accepted by isConvex only -- operators require degree<=2 (see assertOperable).
   %
   % based on
           ...
           for jj=2:nedges
               ...
               if jj<nedges
                   i=iNext;%pivot for the NEXT iteration is the vertex the walk just arrived at; recomputing
                           ...
                           %came from instead (see QuaPar.m's orderEdges, which had the identical bug,
                           %fixed the same way). Deliberately NOT done on the LAST iteration: the
                           %closing-angle code just below needs `i` to still hold ITS value from the
                           %last iteration's own processing (the far vertex of the last edge relative to
                           %the closing vertex `iFirst`) -- overwriting it to iNext (the closing vertex
                           %itself) would collapse that angle's v1 to a zero vector.
               end
           end

...
%obj.print
    ...
    methods (Static)
        ...
        % here via `region.tightenBoundedFacet`'s candidate logic applied to THIS conic only.
            ...
            for ii = 1:nq
                % BOTH signs, not just whichever one quadNullDirsNumeric happens to return: Ad =
                % d'Qd is sign-independent (same for d and -d), but the tie-break's grad
                % condition (Lh.'*d <= tol, in recedesFacet) is NOT -- it flips with the sign of
                % d. Missing the negation here meant a facet whose axis recedes in the direction
                % quadNullDirsNumeric did NOT happen to return was never tested at all. Latent
                % since mechanism 1 first landed (2026-08-27); found while building mechanism 3
                % below, which relies on this candidate list being genuinely exhaustive for two
                % convex facets.
                ndq = region.quadNullDirsNumeric(Qs{ii});
                cand = [cand; ndq; -ndq]; %#ok<AGROW>
            end
            ...
            if nq == 2
                % Mechanism 3: BOTH facets CONVEX (Qi PSD) -- if mechanism 1 above found NO
                % receding candidate, the region is provably BOUNDED, with no arc parametrization
                % needed, REGARDLESS of whether the two conics share an axis or not.
                %
                % THE ARGUMENT. A rank<=1 Qi has one zero eigenvalue (null/axis direction ni) and
                % one nonzero eigenvalue lambda_i = trace(Qi) (the other is 0, so the trace IS
                % the nonzero one). PSD means lambda_i > 0. Along ANY direction d, d'Qi d =
                % lambda_i (ni_hat . d)^2 -- PSD forces this >= 0 EVERYWHERE, so a direction can
                % possibly recede facet i ONLY when it lies EXACTLY on ni (d'Qi d = 0); every
                % other direction makes hi(z+td) -> +inf, ruled out. So the FULL set of
                % directions that could possibly recede BOTH facets is exactly {n1, n2}
                % (different axes: disjoint, so no candidate at all; same axis: the shared line,
                % which may still fail the linear tie-break on one facet in one sense while
                % passing the other facet in the OPPOSITE sense -- e.g. one facet needs
                % y -> +inf, the other y -> -inf). Mechanism 1's candidate loop above already
                % tests both n1 and n2, IN BOTH SIGNS (fixed alongside this mechanism -- see that
                % loop's own comment), against BOTH conics. So its failure to find anything is a
                % genuine PROOF of boundedness for two convex facets, not merely "nothing
                % obvious found": there is nowhere else to look.
                %
                % VERIFIED as two separate, previously-latent BUGS, not hypotheticals:
                % `region([y^2-x,x^2-y],[x y])` (different axes, no linear facets at all) has
                % true max of `y-x` = 0.25 at the smooth point (0.25,0.5); `region([x^2-y-1,
                % x^2+y-1],[x y])` (SAME axis (0,1), but one facet only recedes at (0,1) and the
                % other only at (0,-1)) has true max of `x` = 1 and of `y` = 1, both smooth
                % boundary points, not vertices. Both previously answered `Inf` unconditionally.
                % Both confirmed against brute-force sampling before this landed.
                %
                % Once boundedness is established, the exact value needs no new machinery:
                % `tightenBoundedFacet`'s candidate set (finite vertices plus each conic's own
                % tangency point where grad h_i is parallel to cRow) is ALREADY curve-count-
                % agnostic -- it loops over every quadratic facet, not just one -- so calling it
                % with val=Inf as the starting bound (accepted whenever it finds any finite
                % candidate) reuses it exactly as designed.
                bothConvex = true;
                for ii = 1:2
                ...
                % there, not a small generalization). Mechanism 3 above now resolves every
                % both-convex configuration; this abstains only when at least one facet is
                % CONCAVE, where curvature alone no longer rules out an escape.
                return
            end
